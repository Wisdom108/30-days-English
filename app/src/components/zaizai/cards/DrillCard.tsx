...
import { ShadowReadout } from '../../shared'
...
export default function DrillCard({ data, lesson }: { data: DrillCardPayload; lesson: LessonCtx }) {
  ...
  const record = async () => {
    if (isBusy()) {
      take() // tap-to-stop (also swallows an accidental double-tap)
      return
    }
    setShadow(null)
    setHeard(null)
    ...
    try {
      const t = await take()
      if (!t) return
      const r = scoreShadow(data.text, t)
      setHeard(t)
      setShadow(r)
      const words = r.reference.filter((s) => s.word)
      const missed = words.filter((s) => !s.hit).map((s) => s.text)
      // 在在 one-line verdict — best-effort, stays silent on failure.
      zaizaiChat(
        [{
          role: 'user',
          content:
            `我跟读了 "${data.text}",说成了 "${t}",命中 ${words.length - missed.length}/${words.length} 词` +
            (missed.length ? `,漏掉了 ${missed.join(', ')}` : '') +
            '。用一句话点评我的跟读。',
        }],
        lesson,
      )
      ...
      <div className="mt-3 flex items-center gap-3">
        ...
        {shadow !== null ? (
          <div className="min-w-0">
            <ShadowReadout result={shadow} size="sm" />
