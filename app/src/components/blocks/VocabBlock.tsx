...
import { Badge, Button, Card, CardBody, Callout, Segmented } from '../ui'
      ...
      <CardBody>
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-h2 font-semibold">
            词汇 · <span className="t-num">{words.length}</span> 个高频词
          </h2>
        ...
        {due > 0 && (
          <Callout tone="warning" className="mt-3 items-center">
            <div className="flex w-full items-center justify-between gap-3">
              <span>
                你有 <b className="t-num font-semibold text-fg">{due}</b> 张到期词卡需间隔复习
              </span>
        ...
        {mode === 'browse' && (
          <div className="mt-4 grid gap-2.5 sm:grid-cols-2">
            {words.map((w, idx) => (
              <div key={idx} className="rounded-lg border border-border bg-surface-2 p-3.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-h2 font-semibold text-fg">
                    {w.word}{' '}
                    <span className="text-meta font-normal italic text-fg-dim">{w.pos}</span>
                  </span>
                ...
                <div className="text-meta text-fg-dim">{w.example_zh}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {mode === 'quiz' && card && (
          <>
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <span className="t-num text-sm text-fg-dim">
                {i + 1} / {words.length}
              </span>
              <Button
                variant="secondary"
